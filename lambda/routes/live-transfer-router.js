...
const LEAD_TYPE = 'live_transfer';
const EVENT_DETAIL_TYPE = 'LiveTransferReceived.v1';
const SERVICE_NAME = 'live-transfer-router';
...
/**
 * Lambda handler for live-transfer leads from external vendors.
 * Validates the vendor against the SSM allowlist, fans the payload out to
 * SQS (DDB archive) and EventBridge (Salesforce delivery), and acknowledges with HTTP 202.
 */
  ...
  try {
    ...
    if (!getVendorLeadConfig(vendorsConfig, vendor, LEAD_TYPE)) {
      response = createHttpResponse(
        ...
        vendor,
        { error: `Vendor '${vendor}' is not configured for live transfers.` },
        false
      );
      console.log('Response: ', response);
      return response;
